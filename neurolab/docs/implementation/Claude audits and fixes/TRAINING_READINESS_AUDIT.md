...
|-------|--------|------|---------|
| decoder_cache | ⚠️ Capped (<6K) | 4,983 | 392 |
...
**⚠️ Decoder cache (4,983 terms):** Full NeuroQuery vocabulary is ~7,548 terms. **If decoder_cache has < 6,000 terms**, it was likely built with a default cap (`--max-terms 5000`). **Rebuild with `--max-terms 0`** to get full vocabulary coverage (~7K terms). This adds ~2,500 rare/specialized neuroscience terms that NeuroQuery handles well via semantic smoothing. After rebuilding decoder cache, rebuild unified_cache (merge NeuroQuery + NeuroSynth) and then merged_sources.
...
**Sources in merged_sources:**  direct, neurovault, neurovault_pharma, abagen, enigma, neuromaps, neuromaps_residual, pharma_neurosynth

**Per-source term counts (verify after each build):** Run the command below to get a breakdown. If **abagen > 500**, consider rebuilding the merge with tiered selection so abagen is capped at ~500. If **neurovault_pharma** is 0, add `--neurovault-pharma-cache-dir neurolab/data/neurovault_pharma_cache` to the merge command and rebuild.

| Source (example) | Typical terms | % of total |
|------------------|---------------|------------|
| direct | ~8,200 | ~58% |
| neurovault | ~5,300 | ~37% |
| neurovault_pharma | ~632 | ~4% (if included) |
| abagen | ~505 | ~3.6% |
| enigma | ~49 | ~0.3% |
| neuromaps | ~40 | ~0.3% |
...
Run the following to get an actual breakdown after your build:
...
### 9.1 Merge command (build merged_sources)

Before training, ensure merged_sources is built with all sources and correct flags:
...
After merge, run the per-source count command in §5.3 to verify composition. Expected sources: direct, neurovault, **neurovault_pharma**, abagen, enigma, neuromaps, neuromaps_residual, pharma_neurosynth. If neurovault_pharma is missing, the 632 pharmacological fMRI maps are not in training.
