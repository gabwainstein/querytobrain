...
**PET residual maps (at merge):** For each **neuromaps** PET map (and each **receptor** map when receptor is merged), the merge step computes: **residual = map − projection of map onto the first K abagen gradient PCs** (from `--abagen-add-gradient-pcs K`). These are added as **separate** terms with suffix `_residual` (e.g. `5HT2A_PET_residual`) and source = `neuromaps_residual` or `receptor_residual`. Purpose: train the model to predict **pharmacologically specific** spatial patterns beyond dominant transcriptional gradients, so it does not learn the shortcut "anything serotonergic → produce cortical gradient."

**Re-normalization of residuals:** After regressing out gradient PCs, residual variance is reduced (some signal removed). To keep residual maps on a comparable scale with other training targets, **re-z-score residuals** (cortex/subcortex separate, matching PET normalization) after computing the residual. Without this, residuals have systematically lower variance → artificially low MSE → model learns to predict near-zero. Sample weight = 0.6. Enable with `--add-pet-residuals`; requires abagen merged with `--abagen-add-gradient-pcs` &gt; 0.
...
| neuromaps_residual / receptor_residual | merge (from PET maps + gradient PCs) | Residual = map − gradient projection; **re-z-scored** (cortex/subcortex separate) |
...
Used when building `term_sample_weights` for terms that don't have a per-term weight (e.g. from NeuroVault's pkl). **neuromaps** and **receptor** are set to **1.0**: they are ~40 ground-truth PET receptor density maps and the highest-value pharmacological targets; with only ~5% sampling fraction, 1.0 prevents washout.
...
| neurovault | 0.8 | |
| ontology | 0.6 | |
| neuromaps | **1.0** | ~40 PET maps; scarce, high-value pharmacological ground truth. 1.0 prevents washout with only ~5% sampling fraction. |
...
- Optional text augmentation (paraphrases / LLM variants): same map for multiple text variants; sources and weights inherited. **Not yet implemented** in the default pipeline; when added, augmented terms should be placed in the **train** split only (not val/test) to avoid data leakage.
...
- Loss: for batch indices \(B\),  
  ...
  (weighted MSE). So high-weight terms (e.g. meta-analytic NeuroVault) contribute more to the gradient. **Neuromaps and receptor** default to **1.0** so the ~40 PET maps are not washed out despite low sampling fraction; abagen stays at 0.4 with ~10% batch share so gene maps don't dominate.
...
3. Merge: `build_expanded_term_maps.py` with all cache dirs, `--save-term-sources`, and abagen options (e.g. `--max-abagen-terms 500 --abagen-add-gradient-pcs 5`). Output = merged cache dir (e.g. `merged_sources/`).
