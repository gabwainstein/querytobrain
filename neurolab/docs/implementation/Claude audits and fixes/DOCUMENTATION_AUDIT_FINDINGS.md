...
| # | Severity | Doc | Issue | Status |
|---|----------|-----|-------|--------|
| 1 | 🔴 Error | BUILD_METHODS | gradient_components shape (392,K) should be (K,392) | **FIXED** |
| 2 | 🔴 Missing data | BUILD_METHODS, AUDIT | neurovault_pharma (632 maps) not in merge command | **FIXED** |
| 3 | 🔴 Silent loss | AUDIT, CRITICAL_PATH | Decoder cache has 4,983 terms, should be ~7K | **FLAGGED** in audit; rebuild required |
| 4 | 🟡 Missing | AUDIT | No per-source term count breakdown | **FIXED** (command + interpretation added) |
| 5 | 🟡 Legacy | accuracy-and-testing | Entirely 400-D/Schaefer, no 392, no Fulcher eval | **FIXED** (full rewrite) |
| 6 | 🟡 Missing | BUILD_METHODS | PDSP end-to-end processing not documented | **FIXED** (§8.2 added) |
| 7 | 🟡 Missing | BUILD_METHODS | Term collision policy for different data types | **FIXED** (§9.3 expanded) |
| 8 | 🟡 Missing | AUDIT | No merge command shown, only training command | **FIXED** (§9.1 added) |
| 9 | 🟢 Improve | Both weight tables | neuromaps should be 1.0, not 0.8 | **FIXED** (bumped to 1.0) |
| 10 | 🟢 Improve | PREPROCESSING | PET residual maps may need re-z-score | **FIXED** (re-z-score now specified) |
| 11 | 🟢 Improve | abagen_tiered | "WGCNA" label overstates implementation | **FIXED** (renamed to co-expression cluster medoids) |
| 12 | 🟢 Improve | BUILD_METHODS | 500 abagen should be ratio-justified, not magic number | **FIXED** (ratio note added) |
| 13 | 🟢 Improve | abagen_tiered | Cross-normalization dependency not documented | **FIXED** (warning added) |
| 14 | 🟢 Improve | PREPROCESSING | Text augmentation mentioned but unspecified | **FIXED** (flagged as not yet implemented) |
| 15 | 🟢 Improve | AUDIT | No expected baseline metrics for new pipeline | **FIXED** (in accuracy-and-testing §4) |
