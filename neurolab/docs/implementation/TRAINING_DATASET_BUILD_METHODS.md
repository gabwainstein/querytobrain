...
When merging, we cap gene maps to avoid overweighting. Selection is **tiered** (literature-aligned): see [abagen_tiered_gene_selection.md](abagen_tiered_gene_selection.md). **Recommended total: ~500** (not 2000). The full build uses `--max-abagen-terms 500`. The number is chosen so abagen is ~3–5% of the training set; adjust if other sources change significantly (e.g. after adding neurovault_pharma, 500 abagen remains a good ratio).
...
- **`residual_variance`** (default): Rank genes (excluding Tier 1/2) by variance of residual after regressing out **top 3–5 PCs**; take top N − (Tier1 + Tier2). Informed by Fulcher et al. (genes redundant with dominant gradients add little). Typical Tier 3 count: **~200** when N = 500.  
- **`medoids`**: Cluster remaining genes; one medoid per cluster.

**Summary:** Tier 1 ≈ 250 receptor genes + Tier 2 ≈ 30 WGCNA hub genes + Tier 3 ≈ 200 residual-variance genes ≈ **~500 total**. This avoids abagen dominating; source-weighted sampling (abagen ~10%) and loss weight 0.4 then balance effectively.

**Gradient maps (recommended default):** With **`--abagen-add-gradient-pcs K`** (recommend **3**), the first K PCs are added as **separate synthetic terms** with semantic labels and saved as `abagen_gradient_components.npy` for PET residual-correlation evaluation. PC1–3 are robust; PC4–5 often less generalizable. Full build passes `--abagen-add-gradient-pcs 3`.

**Gradient PC label style:** `--gradient-pc-label-style distinct` (default in run_full_cache_build) uses optimized labels: PC1 = "Cortical gene expression gradient: sensorimotor-association axis (primary organizational axis)", PC2 = "Metabolic gradient: oxidative vs. synaptic pathways", PC3 = "Developmental gradient: adolescent plasticity and neuroimmune axis". Other styles: current, hybrid, standard, brain_context, distinctive, short, dominant.

**Implementation:** `build_expanded_term_maps.py` (merge step); options `--max-abagen-terms` (recommend 500), `--abagen-n-clusters` (default 32), `--abagen-add-gradient-pcs` (recommend 3), `--abagen-tier3-method` (default residual_variance), `--abagen-gene-info` (for label enrichment; run `download_gene_info.py` first), `--abagen-pca-variance 0.95` (denoise gene maps).
...
### 8.1 End-to-end pipeline (preprocessing → spatial maps)

1. **Download:** `download_pdsp_ki.py` fetches KiDatabase.csv from PDSP. If automated download fails, download manually from pdsp.unc.edu and place in `neurolab/data/pdsp_ki/`.
2. **Process (optional):** `process_pdsp_for_neurolab.py` (if used) converts Ki (nM) → pKi, filters species (human when available), and builds a compound × receptor affinity matrix. PDSP receptor names are mapped to gene symbols via **RECEPTOR_TO_GENE**; not all PDSP receptor names have a mapping — unmapped receptors are skipped. Validation: check how many compounds have ≥3 mapped receptors and how many gene symbols are covered (~60–80 typical).
3. **Gene PCA Phase 1–2:** `run_gene_pca_phase1.py` and `run_gene_pca_phase2.py` produce the gene-expression basis in `gene_pca/` (same normalization as abagen/neuromaps: cortex/subcortex separate). Required for projecting drug profiles to 392-D.
4. **Build PDSP cache:** `build_pdsp_cache.py` loads the Ki CSV, builds per-compound binding profiles (receptor → 1/Ki or pKi-based weight), maps receptors to genes via RECEPTOR_TO_GENE, projects each profile through gene PCA to get 392-D spatial maps. Output: `pdsp_pc_projections.npz`, compound names, metadata. **Expected:** ~3K–5K compounds with ≥3 receptors mapped; ~60–80 gene symbols from RECEPTOR_TO_GENE.
...
Terms are deduplicated by **normalized label** (lowercase, strip, collapse whitespace). **First occurrence wins:** the order of merging (base cache → neuromaps → neurovault → neurovault_pharma → pharma_neurosynth → receptor → ENIGMA → abagen → residuals) determines which map is kept when the same label appears in multiple sources.

**Collision caveat:** A term like "dopamine" can exist in **direct** (NeuroQuery text-mined meta-analysis), **pharma_neurosynth** (MKDA of studies mentioning dopamine), and **neuromaps** (PET dopamine receptor density). These are different data types (fMRI vs PET). Currently the NeuroQuery version wins because the base cache is merged first, so the PET and pharma meta-analysis maps for that label are dropped. **Recommendation:** When building caches, use distinct labels for non-fMRI sources where possible (e.g. `5HT2A_PET` instead of `5HT2A` for neuromaps). Optionally log which terms are dropped at merge time to verify that PET/receptor maps are not lost. Future options: namespace prefixing for non-fMRI sources, or allowing multiple maps per term with different `map_type`.
...
| direct | 1.0 | |
| neurovault | 0.8 | |
| neuromaps | **1.0** | ~40 PET maps are scarce, high-value pharmacological ground truth; 1.0 so they are not underweighted (source-weighted sampling already limits batch share). |
...
**Gradient PC labels:** `--gradient-pc-label-style distinct` uses optimized labels: PC1 (brain_context phrasing for best recovery), PC2/3 (maximally different phrasing). Run `download_gene_info.py` first for `gene_info.json`. Optional: `--additional-abagen-cache-dir` for receptor residual-selected denoised genes when that cache exists.
...
| `abagen_gradient_components.npy` | (K, 392) float64 | Gradient PCs when `--abagen-add-gradient-pcs` used (sklearn convention: n_components × n_features); for PET residual-correlation evaluation |
...
12. Merge → `merged_sources/` with `--no-ontology`, `--save-term-sources`, `--abagen-add-gradient-pcs 3`, `--add-pet-residuals`, and all available cache dirs (neuromaps, neurovault, neurovault_pharma, pharma_neurosynth, enigma, abagen)
